...
- ## if 语句
	...
	- **基本语法：**
		- ```go
		  ...
		  ```
	- **`if` 之前可带短语句：**
		- 允许你在 `if` 条件判断之前，先执行一个“初始化语句”，通常是一个短变量声明，这个变量在当前分支和所有后续分支中都可用，一旦 `if-else` 块结束，变量就会被销毁。
		- **这种设计的好处是：**
...
- ## switch 语句
	- **基本语法：**
		- ```go
		  ...
		  ```
		- **表达式可以放什么？**
			- 只要一个数据类型支持 `==` 运算符（即它是可比较的），它就可以放在 `switch` 后面。
			- **基本类型都可以；内部元素可比较的复合类型也行。**
				- 整数与浮点数：如 `int`、`int64`、`float64`。
				- 字符串：`string`。
				- 布尔值：`bool`。
				- 复数类型：`complex128`。
				- 数组（Array）：如 `[3]int`。
				- 结构体（Struct）：如 `struct{ x int }`。但如果结构体中包含切片或 map 字段，则无法比较。
				- 指针：比较其指向的内存地址。
				- Channel：判断是否是同一个 channel。
				- 接口（Interface）：比较动态类型与动态值。
			- **注意：切片（Slice）、`map`（映射）和 `func`（函数）是不可比较的。**
			- **函数的返回值：**你可以把一个函数调用放在 `switch` 后面。此时，Go 会立即执行一次该函数，并使用它的返回值去和 `case` 进行匹配（的返回值类型必须是可比较的）。
	...
	- **无条件的 `switch`**
		- **语法**：`switch { ... }`
		- **行为**：表达式默认为 `true`。`switch` 会从上到下查找**第一个 `case` 表达式为 `true` 的分支**，执行它，然后退出。
		- ```go
		  ...
		  ```
		- **比 `if-else` 链更好：**
			- **用法**：这种写法非常适合替代冗长的 `if-else if-else` 链。此时 `case` 后面必须跟随一个返回布尔值的**条件表达式**。
			- 这种 `switch { ... }` 结构比 `if-else` 链更易读，因为你不需要重复写 `else if`，所有的条件都整齐地对齐了。
			- ```go
			  if t.Hour() < 12 {
			      fmt.Println("Good morning!")
			  } else if t.Hour() < 17 {
