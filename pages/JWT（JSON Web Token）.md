...
- **JWT 的结构**
	...
	- **Payload（载荷）：**
		...
		- **JWT 的规范将声明分为三类：**
			- **注册声明：**
				- JWT 规范预定义的一组标准声明，它们不是强制性的，但推荐使用。
				- 这些声明的键名都是单词前三个字母的缩写。
				- `iss`：Issuer，签发者
					- 谁颁发的 Token。接收方可以用这个值来验证签发者是否是它所信任的。
				- `sub`：Subject，主题
					- Token 代表的用户/实体。这个令牌代表的是哪个用户，通常是用户的 ID。
				- `aud`：Audience，受众
					- Token 的目标接收方。如果一个服务收到了一个 `aud` 不包含自己的 JWT，它应该拒绝该令牌。这可以防止一个为服务 A 签发的令牌被恶意用到服务 B 上。
