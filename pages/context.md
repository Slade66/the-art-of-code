...
- **`context.Context` 的三大核心作用：**
	...
	- **传递请求范围的值（Request-scoped Values）：** 在请求处理链中，`Context` 用于传递与请求相关的元数据，如用户 ID 和请求唯一标识（`trace_id`），无需显式地将这些值作为函数参数逐层传递。
- **注意：**
	...
	- 不要将 `Context` 存储在结构体中，应将其作为函数的第一个参数显式传递，通常命名为 `ctx`。
- API
  heading:: true
	- `context.Background()`
