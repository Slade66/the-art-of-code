...
heading:: true
	- **API 优先的设计理念：**
		- 首先，在 `.proto` 文件中使用 Protobuf 接口定义语言来定义服务接口、消息数据结构和错误码。接着，通过工具生成 gRPC 和 HTTP 的服务端/客户端骨架代码。这种方式在微服务架构中广受推崇，因为 API 契约是跨语言、跨团队协作的基石。
	-
- Kratos 项目的目录结构
  heading:: true
	...
	- `internal`：存放项目主要业务逻辑的目录。
		...
		- `biz`：业务逻辑层，存放核心业务逻辑并被 service 层调用。在 biz 层定义 Repo 接口，规定数据操作的要求（接口），由 `data` 层实现。biz 层只关心调用方法，而不关心数据的来源。
		- `data`：数据访问层，负责操作数据库。它实现了 `biz` 层定义的 `Repo` 接口，为 `biz` 层提供数据支持。
		- `service`：服务层，充当“胶水层”，将外部请求与内部业务逻辑连接起来。它接收 gRPC 或 HTTP 请求，进行简单的参数处理后，调用 `biz` 层执行业务逻辑。
