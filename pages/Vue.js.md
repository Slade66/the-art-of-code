...
- Vue 的文件结构：
	- `<template>`：定义组件的结构。
	- `<script setup lang="ts">`：定义组件的行为、逻辑。
	- `<style>`：定义组件的样式。
	  collapsed:: true
		- `scoped`：意思是这里的 CSS 只对当前这个文件有效。即使你在别的地方也有个同选择器的样式，也不会互相冲突。
- `defineProps`：
  collapsed:: true
	- ```typescript
	  <script setup lang="ts">
	  defineProps({
	    name: String, // 定义了一个叫 'name' 的属性，类型是字符串
	  });
	  </script>
	  ```
	- 这是子组件向父组件“要数据”的方式。它在说：“谁想用我，必须给我传一个叫 `name` 的字符串，我会把它显示在 `<template>` 里的 `{{ name }}` 里。”
-
-