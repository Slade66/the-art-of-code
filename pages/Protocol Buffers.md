概念
heading:: true
	- 简称 Protobuf，是 Google 开发的一种与语言无关的，用于定义数据结构和服务接口的接口描述语言（IDL）。它根据 proto 文件中的定义，生成多种编程语言对应的数据结构操作代码（进行序列化与反序列化），支持在不同服务之间传输结构化数据，从而实现跨语言和系统的数据通信。
...
- ## Protobuf 的命名规范
	...
	- **service：**
		- 推荐格式：`<Resource>Service`
		- RPC 方法命名：动词（Create、Get、List、Update、Delete）+ 资源
	- 在 Protobuf 的同一命名空间（同一 package）内，顶层符号名必须唯一。`service` 和 `message` 也共享同一个符号表，所以不能同名。
- ## 选项（Option）
  collapsed:: true
