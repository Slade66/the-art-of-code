	...
	- **什么时候用切片的指针接收者？**
		...
		- **例子：**
			...
			- ```go
			  ...
			  ```
			- **`Todos` 值接收者改的是副本，`*Todos` 指针接收者改的是原对象。**调用看起来一样，但是否能“真正改到外面的切片”差别巨大。
			...
			- `t.AddPtr(Todo{Title: "B"})` 这一行一步步拆开看，看看 Go 语言底层到底做了什么：
				...
				- 现在 `todos` 指向外部变量 `t` 的地址。
					- ```go
					  todos  --->  t  --->  底层切片结构（ptr, len, cap）
					  ```
				- 当你执行：`*todos = append(*todos, t)`
					- `*todos` 取出 `t` 的值，也就是一个空切片；
					  logseq.order-list-type:: number
					- `append(*todos, t)` 会创建一个新切片（长度从 0 变成 1）；
					  logseq.order-list-type:: number
					- `*todos = ...` 再把这个新切片赋值回原来的变量 `t`。
					  logseq.order-list-type:: number
					- 也就是说，这次 `append` 的结果直接回写给了外部变量 `t`。
- **`append`  操作：**
