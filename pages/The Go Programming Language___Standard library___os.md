...
- `func Exit(code int)`
  collapsed:: true
	- **作用：**使当前程序以指定的状态码退出。按照惯例，状态码 0 表示成功，非零值表示错误。程序会立即终止，延迟函数（`deferred` 函数）不会执行。
	- **什么是退出码？**
		- 当你的 Go 程序运行完后，操作系统会接收到一个整数返回值，叫做退出码。
			- 0，正常退出
			- 非 0，异常退出，代表程序出错
		- **Go 程序默认的退出码：**如果你写的 `main()` 函数正常执行结束（没有 `panic`，也没调用 `os.Exit()`），Go 会自动以 退出码 0 结束。
		- **退出码与 `panic` 的关系：**如果程序触发了 `panic` 且没有被 `recover()` 捕获，Go 会打印堆栈信息并以退出码 2 结束。
	- **退出码的作用：**
		- **Shell 脚本判断是否执行成功**
			- ```bash
			  go run main.go
