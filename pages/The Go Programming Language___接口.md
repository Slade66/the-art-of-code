- **接口是什么：**
  collapsed:: true
	- 接口是一个标准规范，用于让两个不同的事物合作。如果没有统一的标准，大家各搞各的，最后在集成时就会出现对接不上的情况。
	- 接口不关心数据，接口只关心行为，它定义了一组方法签名（方法名、参数、返回值）的集合。
- **接口有什么用？**
  collapsed:: true
...
- **类型断言：**
  collapsed:: true
	...
	- **Type Switches**
	  id:: 6932808f-4fd4-41f7-9f37-0e309c77aeef
		...
		- `switch v := i.(type) { ... }`，这是固定语法。
			- `接口变量.(type)` 是一个只能在 `switch` 语句中使用的特殊语法。它用于告诉 Go：“我要判断变量的具体类型。”
			- **示例代码：**
				- ```go
				  package main
...
- **接口变量**
  collapsed:: true
	...
	- **接口字段直接使用值类型：**接口本身具有引用语义，已经能够指向具体的实现类型，因此一般没必要再使用“接口的指针”。那样不仅没必要，反而会多出一个间接层。
	- 可以给接口变量传入实现类型的值的指针，编译器会自动对指针进行解引用获取值。
	- **接口变量里装了什么？**
		- 一个接口变量就是一个**包含两个插槽的容器 `(value, type)`**：
			- **`type` (动态类型)：** 存放值的具体类型信息。
			- **`value` (动态值)：** 存放该类型的实际值的地址。
			- 你可以把它想象成一个容器，这个容器的静态类型是接口类型，但它实际持有的内容（值和类型）是动态变化的，只要这个实际持有的类型实现了这个接口所定义的所有方法。
