...
- [[git pull]]
- ## 问题
	- **为什么同事在她的 Git 中看不到我已经推送的远程分支，却能看到我很早之前就已经删除的旧分支？**
	  collapsed:: true
		- **问题现象：**她执行了 `git pull` 和 `git pull origin main`，但依然看不到我推送的那个分支。
		- **问题原因：**
			- **为什么看得到旧分支？**
				- 当你删除远程分支 `origin/old-feature` 后，服务器上确实已经不存在了。但她之前 fetch 过这个分支，于是本地生成了一个引用文件。Git 默认不会主动删除这些本地的 `origin/xxx` 引用，即使远程已经移除，这是为了避免意外删除导致的数据丢失。
				- 所以，她看到的“远程分支”，其实只是本地硬盘里遗留的一个已经过期的缓存文件。
				- 因为 Git 的设计原则是：除非有明确指令，否则不会主动删除数据。虽然远程已经删掉了，但她本地的“远程分支缓存”仍然保留，需要手动进行 prune。
			- **为什么看不到新分支？**
				- 因为 `git pull origin main` 是一个指定目标的操作。它只是从远程抓取 `main` 的最新数据，将抓取到的数据合并到当前分支，并没有收到指令去查询“远程还有哪些其他新分支”，所以她的本地 Git 还没意识到新分支的存在。
		- **解决方案：**
			- **执行：**`git fetch -p` 或 `git fetch --prune`。
			- **Fetch（抓取）：**它会连接远程仓库，下载你新推送的分支信息，用来解决“看不到新分支”的问题。
			- **Prune（修剪）：**`-p` 参数会让 Git 对比远程仓库，如果某些分支已经被删除，就把本地对应的 `origin/xxx` 缓存一并清理，从而解决“还能看到旧分支”的情况。
