...
- **语法：**
	...
	- `cleanWs()`：
		- **作用：**它的全称是 Clean Workspace，清空当前节点当前任务的工作目录。
		- **为什么要使用它？**
			- 当你运行一个 Jenkins 任务时，Jenkins 会在执行节点上创建一个文件夹来存放从 Git 拉取的代码、编译生成的二进制文件以及中间产物。如果不使用 `cleanWs()`，下一次构建时，这些旧文件会依然存在。
			- **确保环境纯净：**防止上一次构建残留的旧代码、旧配置文件或错误的 `bin/` 产物干扰本次构建。
			- **节省磁盘空间**：如果不清理，成百上千次的构建产物（尤其是大的 Docker 镜像层和二进制文件）会迅速填满磁盘。
		- **在流水线中放哪里？**
			- **在最开始执行：**在 `Checkout` 阶段之前执行，确保下载代码前目录是空的。
			- **在结束后执行（Post 块）：**无论构建成功还是失败，都清理现场，不给宿主机留垃圾。
- **Shared Library：**
