...
- **字段标签：**
	...
	- `primaryKey`：指定该字段是主键。
		- 如果你的主键不叫 `ID`，就需要这个标签。
		- 如果有两个 `primaryKey`。GORM 会自动将它们组合成一个复合主键。
	- `unique`：指定该字段在数据库中必须是唯一的。使用该标签后，GORM 会在创建数据库表时为该字段创建唯一索引，不允许列中出现重复的值。
	- `autoCreateTime`：当这条记录被创建时，GORM 会自动填入当前时间。
	- `index:idx_name`：你可以使用 `index` 标签为某个字段创建普通索引，大幅提升查询速度。
	- `uniqueIndex:uk_name`：创建唯一索引。
	- `default`：
