...
- **值接收者和指针接收者：**
	...
	- **最佳实践：**如果方法需要修改结构体实例的字段，必须使用指针接收者。即使不需要修改，使用指针接收者（特别是对于大结构体）也能避免不必要的复制，提高性能。
