...
- **API 网关解决了什么问题？**
	- 在微服务系统中，有一些功能是所有服务都需要的，它们被称为 “横切关注点”，例如：
		- **认证（Authentication）**：验证用户身份，例如检查 JWT Token。
		- **授权（Authorization）**：判断用户是否具备访问目标资源的权限。
		- **限流（Rate Limiting）**：控制用户在一定时间内的访问频率，防止系统过载或被恶意请求拖垮。
		- **日志（Logging）**：记录和追踪请求的访问情况，用于系统监控与问题排查。
	- **网关的作用：** 与其让每个微服务分别实现认证、限流等通用功能，不如在系统的第一道入口——API 网关统一处理。这样，只需在网关层维护一次安全与流量控制逻辑，就能覆盖所有 API 请求的非业务性需求。简化了微服务的开发与维护，使各个服务能够专注于自身的核心业务逻辑。
- TODO: APISIX 架构
- **Apache APISIX 的端口：**
