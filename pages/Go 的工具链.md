...
- `go build`
  collapsed:: true
	...
	- **原理：**
		- `go build` 是静态链接，生成的二进制文件不依赖目标服务器的库，扔过去就能跑。
	- **适用于**：
...
- `go generate`
  collapsed:: true
	- **作用：**
		- `go generate` 会扫描你项目中的 Go 源文件，找出一种特殊的注释 `//go:generate` 后面的命令，并开启一个子进程执行。
	- **怎么用：**
		...
		- `go generate ./...`：
		  collapsed:: true
			- 从当前目录开始，递归地扫描所有子目录下的 Go 源文件，打开所有 `.go` 文件，逐行查找 `//go:generate` 注释，并执行它们。
