...
- `go get`
  collapsed:: true
	- **作用**：
		- 用于获取远程依赖包，并将它们加入当前项目的模块中（写入到 `go.mod` 文件）。
		- 可以把它理解成 Go 的“依赖安装命令”，作用类似于：
			- Node.js 的 `npm install`
			- Python 的 `pip install`
	- **原理：**
		- `go get` 命令会根据提供的包路径，从网络上下载包（使用 Git 拉取源代码），并下载到本地的模块缓存目录（`$GOPATH/pkg/mod/`）。同时，`go.mod` 和 `go.sum` 文件也会更新，记录版本和校验信息。
	- **参数：**
		- `-u`：
			- 会把包（以及它依赖的其它库）升级到当前主版本下的最新版本，让整个依赖树都使用较新的版本。`v1.23.0` → `v1.27.0`。
			- **注意：**
				- `-u` 不会升级到新的主版本（如 v2 → v3）。
				- 建议在生产项目中谨慎使用 `-u`，先确认新版本是否兼容。
			- `-u=patch`：
				- 你只想升级小补丁版本，而不动次要版本。
				- 假设当前版本是 `v1.23.1`，那只会升级到 `v1.23.5`（不会升到 `v1.24.0`）。
	- **常见原因和解决办法**：
		- **网络问题**：
