...
- **你几乎不会直接使用数组。你会一直使用切片**
  collapsed:: true
	- 数组是实实在在的数据块。切片只是一个很小的结构体（指针+长度+容量）。
		...
		- **示例：传递数组指针**
		  collapsed:: true
			- ```go
			  ...
			  ```
	- **可以使用相等运算符 `==` 和不等运算符 `!=` 来比较两个数组：**
	  collapsed:: true
		- 当使用 `==` 运算符时，只有当两个数组中**所有对应索引位置**的元素都相等时，结果才为 `true`。
		- 如果使用 `!=` 运算符，只要有一个对应位置的元素不相等，结果就为 `true`。
		- ```go
		  package main
		  ...
		  func main() {
		      ...
		      a := [3]int{1, 2, 3}
		      b := [3]int{1, 2, 3}
		      c := [3]int{1, 2, 4} // 最后一个元素不同
		  
		      // 数组类型: [4]int (长度不同)
		      d := [4]int{1, 2, 3, 0}
		      
		      // 数组类型: [3]string (元素类型不同)
		      e := [3]string{"a", "b", "c"}
		  
		      // 比较 a 和 b (类型相同，所有元素相等)
		      fmt.Printf("a == b: %v\n", a == b) // 输出: a == b: true
		  
		      // 比较 a 和 c (类型相同，但有一个元素不相等)
		      fmt.Printf("a == c: %v\n", a == c) // 输出: a == c: false
		      fmt.Printf("a != c: %v\n", a != c) // 输出: a != c: true
		  
		      // 编译错误：不同类型的数组不能比较 (长度不同)
		      // fmt.Printf("a == d: %v\n", a == d) 
		  
		      // 编译错误：不同类型的数组不能比较 (元素类型不同)
		      // fmt.Printf("a == e: %v\n", a == e) 
		  }
		  ```
		- **只有当数组的类型（元素类型和长度）都相同时，才能进行比较。**
		  collapsed:: true
			- 例如：`[3]int` 类型的数组只能与另一个 `[3]int` 类型的数组比较；它不能与 `[4]int` 或 `[3]string` 类型的数组比较。
		- **注意：切片（Slice）与数组不同。切片不能直接使用 `==` 或 `!=` 运算符进行比较。**
		  collapsed:: true
			- 如果要比较两个切片是否相等（长度相同且所有元素相等），你需要使用 `reflect.DeepEqual()` 函数或手动遍历所有元素进行比较。
- len 函数返回数组的长度。
-