有什么用？
heading:: true
	- 传统的锁机制因读写阻塞而存在并发性能瓶颈。MVCC（Multi-Version Concurrency Control）在不牺牲数据一致性的前提下，提高了数据库在处理并发事务时的能力，特别适用于读多写少的场景。
	- MVCC 主要用于快照读，它允许在不加锁的情况下，读取事务开始时或语句开始时的数据快照。
...
- 实现原理
  heading:: true
	...
	- Undo Log
	  heading:: true
		- 行数据的版本链，记录了数据的历史版本。
		- InnoDB 会为每一行数据维护多个版本。在事务进行快照读时，系统会根据事务 ID 和可见性规则，查找在该事务看来“可见”的数据版本。当数据被修改时，系统不会直接覆盖旧数据，而是创建一个新版本，并让其指向旧版本。
	- Read View
	  heading:: true
